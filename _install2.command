...
#####################################
# IMI Max patches Installation v2.30
# Nicolas Marechal
# n.marechal@lcc.arts.ac.uk
#####################################

function do5Installation {
	echo "- Installing IMI tools for Max 6 or Max 6.1";
	echo ""
	echo "- Quitting Max..."
	killall Max;
	echo ""
...
function doInstallation {
	...
	if [ -e "$C74Examples" ] ; then
		...
		cp -R "$InstallFolder/Packages/scripts" "$C74Examples/IMIpatches/scripts/";
...
if [ -e "/Applications/Max%206.1" ]; then
...
if [[ $whichVersion == 3 ]]; then
	...
	IMIpatches="$maxAppFolder/packages/IMIpatches/patchers/";
	IMIextra="$maxAppFolder/patches/extras/";
	do5Installation;
fi
